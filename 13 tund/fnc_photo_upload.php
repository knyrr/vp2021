<?php
    ...
    function resize_photo($src, $w, $h, $keep_orig_proportion = true){
		...
		} elseif($keep_orig_proportion){//kui kuvasuhe tuleb säilitada
	...
	function store_news_photo_data($image_file_name){
		...
		echo $conn->error;
		$stmt->bind_param("is", $_SESSION["user_id"], $image_file_name);
		if($stmt->execute()){
			$stmt->close();
			$stmt = $conn->prepare("select id from vpr_newsphotos where filename = ?");
			echo $conn->error;
