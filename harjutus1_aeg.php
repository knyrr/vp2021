<?php
	...
	if ($day_category = "koolipäev"){
		if ($hour_now >= 0 and $hour_now < 8){
			$time_category = "uneaeg";
		} elseif ($hour_now >= 8 and $hour_now < 16){
			$time_category = "kooliaeg";
		} else {
			$time_category = "vaba aeg";
		}
	}
	else {
		if ($hour_now >= 2 and $hour_now < 10){
			$time_category = "uneaeg";
		} else {
			$time_category = "vaba aeg";
		}
...
<body>
	...
	<p>Lehe avamise hetk: <span> <?php echo $weekday_names_et[$weekday_now - 1] .", " . $full_time_now .", on " .$day_category  .", on ". $time_category ?>. </span></p>
	<?php echo $photo_html; ?>
</body>
</html>